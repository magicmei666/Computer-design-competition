...
public class DialogueManager : MonoBehaviour
{
            ...
            if (Input.GetMouseButtonUp(0) && !isScrolling)
            {
                ...
                else//MARKER 对话即将结束时候
                {
                    if (GetQuestResult() && talkable.questable.isFinished == false)//MARKER 如果当前对话的这个任务【已经完成】
                    {
                        ...
                        //for(int i = 0; i < Player_1.instance.questList.Count; i++)
                        //{
                        //    if(Player_1.instance.questList[i].questName == talkable.questable.quest.questName)
                        //    {
                        //        Player_1.instance.questList.RemoveAt(i);
                        //    }
                        //}
                    }
                    else//MARKER 如果当前对话的这个任务【没有完成】
                    {
                        Player_1Movement.instance.isTalking = false;
                        ...
                        if (talkable.questTarget != null)
                        {
                            for (int i = 0; i < Player_1.instance.questList.Count; i++)
                            {
                                if (talkable.questTarget.questName == Player_1.instance.questList[i].questName)
                                {
    ...
    public bool GetQuestResult()
    {
        ...
        for (int i = 0; i < Player_1.instance.questList.Count; i++)
        {
            if (talkable.questable.quest.questName == Player_1.instance.questList[i].questName
                && Player_1.instance.questList[i].questStatus == Quest.QuestStatus.Completed)
            {
    ...
    public void ShowDialogue(string[] _newLines, bool _hasName)
    {
        ...
        Player_1Movement.instance.isTalking = true;
    }

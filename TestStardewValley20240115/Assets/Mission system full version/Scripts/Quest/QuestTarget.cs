...
public class QuestTarget : MonoBehaviour
{
    ...
    public void CheckQuestIsComplete()
    {
        ...
        {
            if (questName == Player_1.instance.questList[i].questName 
             && Player_1.instance.questList[i].questStatus == Quest.QuestStatus.Accepted)
            {
                switch (questType)
                {
                    case QuestType.Gathering:
                        if(Player_1.instance.itemAmount >= Player_1.instance.questList[i].requireAmount)
                        {
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        if (other.CompareTag("Player")) {
            ...
            for(int i = 0; i < Player_1.instance.questList.Count; i++)
            {
                if(Player_1.instance.questList[i].questName == questName)
                {
